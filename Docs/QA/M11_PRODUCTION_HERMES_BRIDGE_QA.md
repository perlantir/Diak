...
## Repeatable provider Canvas proof

```bash
Scripts/diak_provider_canvas_probe.sh
```

The Canvas probe refuses to pass if:

- `/version.mode` is not `production_bridge`.
- The assistant response does not include the unique nonce.
- `/sessions/{id}/canvas/artifacts` does not return a typed Browser artifact.
- The Browser artifact preview does not include the nonce generated by the real runtime.
- The Diak SSE stream does not include `canvas_updated`.
...
Latest provider Canvas evidence:

- `qa/diak-provider-canvas-20260510-122235/DIAK_PROVIDER_CANVAS_PROBE_20260510-122235.md`
- Provider: `openai-codex`
- Model: `gpt-5.5`
- Diak session: `sess-diak-69b6136ce6774d5c`
- Hermes session: `20260510_122236_4bed9a`
- Nonce verified in assistant response and Browser canvas artifact preview: `DIAK_PROVIDER_CANVAS_20260510-122235_26282`
...
- Python bridge tests: **PASS — 7 tests** including production Browser canvas artifact capture.
- App-managed bridge Swift tests: **PASS — 9 targeted tests**.
- Debug macOS build: **PASS**.
- Full macOS XCTest suite: **PASS — 170 tests, 0 failures**.
...
- Latest full test result bundle: `/Users/perlantir/Library/Developer/Xcode/DerivedData/HermesDesktop-bolrhhijfkugdtajbptthtffutoz/Logs/Test/Test-HermesDesktop-2026.05.10_12-25-10--0500.xcresult`.
- `git diff --check`: **PASS**.
